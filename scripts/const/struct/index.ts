#!/usr/bin/env node
/**
 * Const Struct Registry Generator
 * Сканирует src/lib/{domain}/const/struct/** и генерирует
 * src/lib/{domain}/data/json/const/struct/index.json для каждого домена.
 *
 * Использование:
 *   npx tsx scripts/const/struct/index.ts
 */
...
async function main(): Promise<void> {
	...
	console.log('[const-struct] Generating simplified JSON snapshots (Struct format)...');
	console.log('='.repeat(60));

	const domains = getDomainsWithConst(CONST_TYPE);

	if (domains.length === 0) {
		console.log('[const-struct] No domains with const/struct found.');
		return;
	}

	console.log(`\nFound ${domains.length} domain(s) with const/struct:`);
	...
	console.log('\n' + '='.repeat(60));
	console.log(`[const-struct] Done! ${totalTokens} token(s) across ${domains.length} domain(s)`);
	console.log('='.repeat(60));
}

main().catch(console.error);

export { main as runConstStructGenerator };
